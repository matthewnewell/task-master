...
Same conventions as every sibling app: a module-level `db` object, `init_db(app)` that creates
tables then runs additive migrations, and a pragma listener enabling foreign keys + WAL mode.
No Alembic — additive-only migrations in `_MIGRATIONS`.
"""
...
def get_db_path(app) -> str:
    ...
    return os.path.join(data_dir, "taskmaster.db")
